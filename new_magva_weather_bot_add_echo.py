...
def location(message):
    if message.location is not None:  # если передали не пустые данные
        ...
        # === Отправка сообщения ботом пользователю и мне в консоль =============
        bot.send_message(message.chat.id, "Координаты приняты, Ваш адрес:\n" + address_str +
        current_weather + final_forecast)

        # print(forecast_wea_data)

        print('Получены координаты от юзверя ', message.from_user.first_name, ' , он находится '
                                                                              'тут: ' + address_str)
